...
function extractCloudTags(name, html) {
    const tags = new Set();
...
async function discover() {
    ...
    const tagLists = await Promise.all(names.map(async (name) => {
        try {
...
async function writeCache(tags) {
    const cache = { scrapedAt: new Date().toISOString(), tags };
 ...
 */
export async function getCloudModels(force = false) {
    ...
    try {
        ...
        if (tags.length === 0)
            throw new Error("scrape returned 0 tags");
