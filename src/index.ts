...
async function loadStatusMessageId(): Promise<void> {
  try {
...
async function saveStatusMessageId(): Promise<void> {
...
async function reconcileStatusMessages(): Promise<void> {
  try {
    ...
    const ours = messages.filter(
      (m) => m.author.id === client.user!.id && m.embeds.some((e) => e.title === "☁️ Ollama Cloud Status")
    );
    ...
    for (const m of toDelete) {
      try { await m.delete(); } catch { /* ignore */ }
    }
...
function msUntilNextMidnight(tz: string): number {
  ...
  const part = (t: string) => parseInt(parts.find((p) => p.type === t)!.value, 10);
...
async function loadNewThisWeek(): Promise<string[]> {
...
async function refreshModelList(force = false): Promise<void> {
  try {
    ...
    if (modelList.length === 0) throw err;
  }
}
...
async function postOrEditStatus(
  channel: TextChannel,
  embed: EmbedBuilder,
  opts: { content?: string; clearContent?: boolean } = {}
): Promise<void> {
  if (statusMessageId) {
    try {
  ...
  const posted = await channel.send({ content: opts.content, embeds: [embed] }) as Message;
...
async function performFullRefresh(channel: TextChannel): Promise<PollResult> {
  await refreshModelList(true);
  resetBackoffState();
